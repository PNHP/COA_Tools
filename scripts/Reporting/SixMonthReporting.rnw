...
\noindent Copies of the SGCN geodatabase were delivered to the agencies for both updates during this reporting period. Agency copies of the SGCN geodatabase were limited to their jurisdictional species plus terrestrial invertebrates. \\\\
...
\noindent The number of SGCN records \Sexpr{ifelse(nrow(SGCNold_sf)>nrow(SGCN_sf),"decreased","increased")} from approximately \Sexpr{comma(nrow(SGCNold_sf))} to \Sexpr{comma(nrow(SGCN_sf))} records. Although most data sources from which we gather SGCN data saw an increase or no change in the number of records, a decline in SGCN records occurred in eBird, PGC bat, and TREC datasets, data which were not updated with new records during the last 6-month reporting period and had records age beyond the 25-year inclusion cutoff. A spreadsheet indicating the number of records available for each SGCN for the \Sexpr{updatetracker[which(updatetracker$updateName==updateName),"UpdateNameDisplay"]} update can be found at \\
...
if(PUchng>0){
  cat(paste0("The number of planning units with at least one SGCN increased by ",PUchng," from ",nrow(PUcnt_6m)," to ",nrow(PUcount_now)," between the two update time periods."))
} else if(PUchng<0){
  cat(paste0("The number of planning units with at least one SGCN decreased by ",PUchng," from ",nrow(PUcnt_6m)," to ",nrow(PUcount_now)," between the two update time periods."))
} else {
...
@
\noindent A more careful examination of the data shows changes in SGCN richness among individual planning units, indicated by the graph below:
...
\noindent The following table presents a summary of SGCN that are missing from the SGCN x Planning Unit dataset, and therefore are not represented in the COA tool. These include SGCN for which we currently have no extant records within the SGCN database.\\
...
if(length(unique(missingCompare$difference))==1&unique(missingCompare$difference[1])==0){
  ...
  cat("Compared to the previous six months, there are",words(abs(sum(missingCompare$difference))),"fewer SGCN in this data update as a result of data aging beyond the cutoff date.")
} else {
  cat("Compared to the previous six months, there are",words(abs(sum(missingCompare$difference))),"more SGCN in this data update as a result of extant records being added for SGCN that were not previously present in our SGCN database.\\\\")
}
@
