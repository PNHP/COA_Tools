...
\noindent In this update, we increased the number of SGCN from 505 (537 including seasons) in April 2019 to 523 (547 including seasons) in November 2019.  While it appears that 18 SGCN were added into the COA tool for this update, the actual data are a little more complicated—35 SGCN were added, with 17 SGCN removed from the tool. Fifteen SGCN removed were freshwater snails with last observation dates beyond the 25-year cutoff that thus aged out of inclusion in the COA tool.\\  
...
\noindent No major structural changes or alterations their tabular SQLite data has been made for this update. Complete documentation of any code changes can be seen in the versioning information on the COA GitHub website (https://github.com/PNHP/COA_Tools).

\subsection*{Changes in Range Maps}
\noindent The addition of Primary Macrogroup and Occurrence Probability fields in the range maps were added for the Quarter 2 updates.

\subsection*{Tool Error Checking}
\noindent We continue to make investments in QC/QA processes for more efficient COA Tool data preparation. Many of these changes are noted above (e.g., taxonomy, ELCODEs, orphaned records due to bad linking), but checks are being added as issues arise and addressed.

\section*{Other Tool Changes}
\noindent No other major tool changes have been made during this reporting period.

\section*{Future Work}
The following work is planned for future updates as data availability and capacity allow:
\begin{itemize}
 \item Improved taxonomic checking to prevent omission by taxonomy and better deal with synonymy.
 \item Creation of a database to house COA tool data (e.g., taxonomy, ranks, actions, etc.) that will simplify data updating.
\end{itemize} 
