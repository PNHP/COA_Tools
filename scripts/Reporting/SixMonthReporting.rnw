...
for(i in 1:nrow(specieslooper)) {
  cat("\\noindent", specieslooper$taxalist[i], "--- ")
...
\noindent In this update, we increased the number of SGCN from \Sexpr{length(cnt_SGCN6mNoSeason)} (\Sexpr{length(cnt_SGCN6m)} including seasons) in April 2019 to \Sexpr{length(cnt_SGCNnowNoSeason)} (\Sexpr{length(cnt_SGCNnow)} including seasons) in November 2019.  While it appears that \Sexpr{words(length(cnt_SGCNnowNoSeason)-length(cnt_SGCN6mNoSeason))} SGCN were added into the COA tool for this update, the actual data are a little more complicated---35 SGCN were added, with 17 SGCN removed from the tool. Fifteen SGCN removed were freshwater snails with last observation dates beyond the 25-year cutoff that thus aged out of inclusion in the COA tool.\\  

\noindent Of the \Sexpr{prettyNum(PUcnt_total, big.mark=",", scientific=FALSE)} possible planning units, the number of planning units with at least one SGCN present decreased by \Sexpr{ifelse(PUchng<=10, words(PUchng), prettyNum(PUchng, big.mark=",", scientific=FALSE))} from \Sexpr{prettyNum(nrow(PUcount_6m), big.mark=",", scientific=FALSE)} to \Sexpr{prettyNum(nrow(PUcount_now), big.mark=",", scientific=FALSE)} between the two time periods! For both time periods, more than 99\% of planning units report at least one SGCN (known or likely presence). However, a more careful examination of the data shows changes in richness among individual planning units as the graph below indicates:
