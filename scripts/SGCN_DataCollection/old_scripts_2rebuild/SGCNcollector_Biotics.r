...
# drop the geometry
srcf_combined_df <- srcf_combined
st_geometry(srcf_combined_df) <- NULL 
...
#c <- unique(lu_sgcn[c("SNAME", "ELCODE")])
#cppCore_sf <- merge(cppCore_sf, c, by="SNAME") # limit to SGCN
cppCore_sf <- cppCore_sf[which(cppCore_sf$SNAME %in% unique(lu_sgcn$SNAME)),]

# drop the geometry
cppCore_df <- cppCore_sf
st_geometry(cppCore_df) <- NULL

# 
b <- srcf_combined_df
st_geometry(b) <- NULL
b <- b[c("EO_ID","ELCODE","SNAME","SCOMNAME","ELSUBID","OccProb","DataID","LASTOBS_YR","SeasonCode")]
b <- unique(b)

cppCore_df <- merge(cppCore_df[c("EO_ID")], b, by="EO_ID")



# remove the source features for which CPPs have been created
srcf_combined_sub <- srcf_combined_df[which(!srcf_combined_df$EO_ID %in% cppCore_df$EO_ID),]
srcf_combined_sub <- srcf_combined_sub[names(cppCore_df)]

finalBiotics <- bind_rows(srcf_combined_sub, cppCore_df)

cppCore_sf_clean <- cppCore_sf[c("EO_ID", "geom")]
srcf_combined_clean <- srcf_combined[c("EO_ID", "geometry")]
srcf_combined_clean1 <- srcf_combined_clean[which(!srcf_combined_clean$EO_ID %in% cppCore_sf_clean$EO_ID),]
#names(srcf_combined_clean1)[names(srcf_combined_clean1) == "geometry"] <- "geom"
st_geometry(srcf_combined_clean1) <- "geom"
final_sf <- rbind(cppCore_sf_clean,srcf_combined_clean1)

final <- merge(finalBiotics, final_sf, by="EO_ID")
