...
# merge
SGCN1 <- merge(SGCN, lu_sgcn_taxa, by.x="TaxaGroup", by.y="code", all.x = TRUE)
SGCN1$LastObs <- as.numeric(SGCN1$LastObs)
SGCN1 <- SGCN1[which(SGCN1$LastObs>=1970),]

library(lattice)
histogram(~SGCN1$LastObs|SGCN1$taxadisplay, breaks=seq(from=1970,to=2022,by=1), layout=c(5, 5), xlab="Last Observation Year", 
          panel=function(x,...){
            panel.histogram(x,...)  
...
# fix bad elseason codes coming in from models. should only need to do this once, but keep in here just in case
lu_sgcnXpu[lu_sgcnXpu=="IILEP42011_y"] <- "IILEP42010_y"
lu_sgcnXpu[lu_sgcnXpu=="AAAAD01010_y"] <- "AAAAD01090_y"
lu_sgcnXpu[lu_sgcnXpu=="AAABC05010_y"] <- "AAABC05200_y"
lu_sgcnXpu[lu_sgcnXpu=="ARADE01010_y"] <- "ARADE01040_y"
lu_sgcnXpu[lu_sgcnXpu=="IMBIV21160_y"] <- "IMBIV21360_y"

# what's happening here? removing bad bird codes?
...
# remove IMBIV elcode with no match in lu_sgcn table
lu_sgcnXpu <- lu_sgcnXpu[!(lu_sgcn$ELSeason=="IMBIV39112"),]
...
write.csv(lu_sgcn[lu_sgcn$ELSeason %in% sgcn_recordnoaction,],here::here("_data","output",updateName,paste0("MissingActions",updateName,".csv")), row.names=FALSE)
...
SGCNrecordcount <- aggregate(LastObs~ELCODE+DataSource, data=SGCN, FUN=length)
colnames(SGCNrecordcount)[colnames(SGCNrecordcount)=="LastObs"] <- "RecordCount"
SGCN_Max <- aggregate(LastObs~ELCODE+DataSource, data=SGCN, max)
colnames(SGCN_Max)[colnames(SGCN_Max)=="LastObs"] <- "MaxYear"
SGCN_Min <- aggregate(LastObs~ELCODE+DataSource, data=SGCN, min)
colnames(SGCN_Min)[colnames(SGCN_Min)=="LastObs"] <- "MinYear"

SGCNsummary <- merge(SGCNrecordcount, SGCN_Max, by=c("ELCODE","DataSource"))
SGCNsummary <- merge(SGCNsummary, SGCN_Min, by=c("ELCODE","DataSource"))
SGCNsummary <- merge(SGCNsummary, lu_sgcn, by="ELCODE", all.x=TRUE)
SGCNsummary <- merge(SGCNsummary, lu_sgcn_taxa, by.x = "TaxaGroup", by.y = "code", all.x = TRUE)
...
write.csv(SGCNsummary, here::here("_data","output",updateName,paste0("SGCNSummary",updateName,".csv")), row.names=FALSE)
