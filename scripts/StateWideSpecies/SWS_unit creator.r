...
for(i in 1:length(sgcnlist)){
  ...
  arc.write(file.path(here("_data","output","sws","sws_new1.gdb",paste("huc08",sgcnlist[i],sep="_"))),sws_huc08_1a ,overwrite=TRUE)
}
...
for(i in 1:length(sgcnlist)){
  ...
  arc.write(file.path(here("_data","output","sws","sws_new1.gdb",paste("county",sgcnlist[i],sep="_"))),sws_county_1a ,overwrite=TRUE)
}
...
arc.write(file.path("sws_new1.gdb","_HUC08_SGCN"),huc08agg_all ,overwrite=TRUE)

countyagg <- sws_countyagg_cast

countyagg_all <- merge(county_shp,countyagg,by="COUNTY_NAM")
arc.write(file.path("sws_new1.gdb","_county_SGCN"),countyagg_all ,overwrite=TRUE)
